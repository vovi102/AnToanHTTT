...
export default function ApprovalsPage() {
  ...
  return <div className="page-stack"><header className="page-header"><div><p className="kicker">KIỂM SOÁT GIAO DỊCH</p><h1>Hàng đợi phê duyệt</h1><p>Kiểm tra giao dịch được tạo tại quầy trước khi phê duyệt.</p></div><div className="header-stat accent"><small>CHỜ XỬ LÝ</small><b>{pending.length}</b></div></header><Feedback value={feedback} onClose={() => setFeedback(null)} /><section className="panel"><div className="section-heading"><div><p className="kicker">PHÂN TÁCH NHIỆM VỤ</p><h2>Giao dịch đang chờ</h2></div><button className="button secondary" onClick={refresh}>Tải lại</button></div><TransactionList transactions={pending} mode="rbac" roles={user?.roles ?? []} /></section></div>;
}
