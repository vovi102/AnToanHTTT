...
export default function TransactionsPage() {
  ...
  const approve = async (transaction: Transaction) => {
    ...
    try {
      ...
      setFeedback({ kind: mode === "baseline" ? "error" : "success", title: mode === "baseline" ? "Giao dịch viên đã tự duyệt" : "Đã phê duyệt", message: mode === "baseline" ? "Một người vừa tạo vừa phê duyệt giao dịch. Đây là rủi ro khi chưa áp dụng phân tách nhiệm vụ." : "Giao dịch đã được cập nhật." });
    } catch (failure) {
  ...
  return <div className="page-stack"><header className="page-header"><div><p className="kicker">TRUNG TÂM GIAO DỊCH</p><h1>{teller ? "Giao dịch của tôi" : "Tra cứu giao dịch"}</h1><p>{teller ? "Tạo và theo dõi yêu cầu chuyển khoản tại quầy." : "Theo dõi trạng thái giao dịch phục vụ kiểm toán."}</p></div><div className="header-stat"><small>TỔNG GIAO DỊCH</small><b>{transactions.length}</b></div></header>
    ...
    <section className="panel"><div className="section-heading"><div><p className="kicker">DANH SÁCH HIỆN TẠI</p><h2>{teller ? "Yêu cầu gần đây" : "Danh sách giao dịch"}</h2></div><button className="button secondary" onClick={refresh}>Tải lại</button></div>{loading ? <div className="empty-state">Đang tải giao dịch…</div> : <TransactionList transactions={transactions} mode={mode} roles={user?.roles ?? []} busyReference={busy} onApprove={approve} />}</section>
  </div>;
}
