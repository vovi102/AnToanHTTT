...
export default function DemoControlPage() {
  ...
  const refresh = useCallback(async () => { try { setMode((await request<{ mode: SecurityMode }>("/demo/security-mode")).mode); } catch (failure) { const error = failure instanceof ApiError ? failure : new ApiError(0, "Không thể tải chế độ"); setFeedback({ kind: "error", title: "Không thể tải cấu hình", message: error.message, status: error.status }); } }, [request]);
  ...
  const changeMode = async (next: SecurityMode) => {
    ...
    } catch (failure) { const error = failure instanceof ApiError ? failure : new ApiError(0, "Không thể đổi chế độ"); setFeedback({ kind: "error", title: "Không thể đổi chế độ", message: error.message, status: error.status }); }
  ...
  const reset = async () => {
    ...
    catch (failure) { const error = failure instanceof ApiError ? failure : new ApiError(0, "Không thể đặt lại demo"); setFeedback({ kind: "error", title: "Không thể đặt lại", message: error.message, status: error.status }); setBusy(false); }
  };
