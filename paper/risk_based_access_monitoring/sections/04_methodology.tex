...
\textbf{SQL injection.} Với mỗi request, nguyên mẫu kiểm tra tuần tự một danh
sách biểu thức chính quy cho tautology, \texttt{union select}, truy vấn xếp
chồng và SQL comment. Mỗi event sinh nhiều nhất một finding, tại mẫu đầu tiên
khớp; evidence chứa tên mẫu và phần request đã cắt ngắn. Các chỉ báo này là
heuristic có thể giải thích, không phải bộ phòng vệ SQL injection hoàn chỉnh
\citep{OWASPSQLi}.

\textbf{Đoán mật khẩu.} Các event \texttt{authentication} được sắp xếp theo
thời gian và phân nhóm theo $(u,ip)$. Trong từng nhóm, một đoạn thất bại kết
thúc khi có đăng nhập thành công hoặc khi khoảng cách giữa hai event liên tiếp
lớn hơn cửa sổ $W_p$. Một finding \texttt{AUTH-PASSWORD-GUESSING} được tạo nếu
trong đoạn có một cửa sổ trượt đạt ít nhất $N_p$ event thất bại. Với cấu hình
thực nghiệm, $N_p=5$ và $W_p=300$ giây. Finding giữ toàn bộ event IDs của đoạn
để evidence có thể được rà soát.

\textbf{Truy cập trái phép.} Luật trong \eqref{eq:unauthorized-rule} chạy độc
lập với hai luật trên. Các finding sau đó được sắp xếp xác định theo thời gian,
loại rủi ro và rule ID trước khi đưa vào bước chấm điểm.
...
học baseline từ các event đã quan sát trước đó. Bốn tín hiệu có thể giải thích
là: (i) \texttt{CTX-AFTER-HOURS} nếu giờ event nằm ngoài $[h_s,h_e)$; (ii)
...
\begin{figure}[t]
  \centering
  \resizebox{0.98\linewidth}{!}{\input{figures/event_alert_incident.tex}}
  \caption{Luồng từ event sang alert và incident. Alert được phân vùng theo cặp
  người dùng--IP rồi sắp xếp theo thời gian. Trong mỗi phân vùng, một khoảng
  cách lớn hơn cửa sổ $W$ tạo incident mới; các trường thời gian chỉ là
  start/end của đoạn, không phải timeline pháp chứng chi tiết.}
