...
nằm trong thư viện chuẩn. Streamlit là giao diện tùy chọn để tải log, lọc alert
và tải kết quả.
...
\section{Thực nghiệm luồng Nova Bank--pipeline}

Thực nghiệm khởi tạo database, tạo năm lần login thất bại và một lần Teller
truy cập quyền phê duyệt bị từ chối. Adapter xuất toàn bộ audit theo thứ tự tăng
dần, parser đọc lại file và application service phân tích để tạo cả
\texttt{password\_guessing} lẫn \texttt{unauthorized\_access}. File do lệnh
export sinh ra cũng được đưa trực tiếp qua parser. Đây là bằng chứng tích hợp
tối thiểu giữa hai phần của dự án; SQL Injection vẫn cần log HTTP vì audit Nova
Bank không giữ raw request.
